...
public interface SearchSuggestionMapper extends BaseMapper<SearchSuggestion> {
    ...
    @Select(
            "SELECT keyword FROM search_suggestion WHERE keyword LIKE CONCAT(#{prefix}, '%') ORDER BY search_count DESC, last_search_time DESC LIMIT #{limit}")
    ...
    @Update(
            "INSERT INTO search_suggestion (keyword, search_count, last_search_time) VALUES (#{keyword}, 1, NOW()) ON DUPLICATE KEY UPDATE search_count = search_count + 1, last_search_time = NOW()")
