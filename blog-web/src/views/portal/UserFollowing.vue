<template>
  <div class="following-page">
    ...
    <div v-else-if="followingList.length === 0" class="empty-state">
      ...
      <p class="empty-text">暂无关注的用户</p>
      <p class="empty-hint">去发现更多有趣的作者吧</p>
    </div>
    ...
    <div v-else class="user-list">
      ...
      >
        <img
          :src="getUserAvatar({ avatar: user.avatar, nickname: user.nickname, userId: user.id })"
          :alt="user.nickname"
          class="user-avatar"
        />
        <div class="user-info">
          ...
          <p class="follow-time">关注于 {{ formatDate(user.followTime) }}</p>
        </div>
